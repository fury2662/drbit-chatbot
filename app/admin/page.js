...
export default function AdminPage() {
  ...
  async function saveCorrection(fb) {
    const answer = correcting[fb.id];
    if (!answer?.trim()) return;
    setLoading(true);
    // FAQ에 추가
    await fetch("/api/faq", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ q: fb.question, a: answer, source: "feedback" }) });
    // 피드백에 수정 답변 저장
    await fetch("/api/feedback", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: fb.id, corrected: answer }) });
  ...
  async function addFAQ() {
    ...
    setLoading(true);
    await fetch("/api/faq", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ q: newQ, a: newA, source: "manual" }) });
    setNewQ(""); setNewA("");
    await fetchFAQ();
    setLoading(false);
    notify("✅ FAQ가 추가됐어요!");
          ...
          {[...filtered].reverse().map(f => (
            <div key={f.id} style={{ background: "#fff", borderRadius: 8, padding: 16, marginBottom: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
              {editId === f.id ? <>
                ...
                {(f.source || f.addedAt) && (
                  <div style={{ fontSize: 11, color: "#aaa", marginTop: 6, display: "flex", alignItems: "center", gap: 6 }}>
                    {f.addedAt && <span>📅 {f.addedAt}</span>}
                    {f.source && (() => {
                      const s = f.source;
                      let label, bg, color;
                      if (s === "manual") { label = "🖊️ 직접입력"; bg = "#e8f5e9"; color = "#2e7d32"; }
                      else if (s.match(/\.(xlsx|xls|csv)$/i)) { label = "📊 엑셀"; bg = "#e3f2fd"; color = "#1565c0"; }
                      else if (s.match(/\.(pdf|pptx|ppt|docx|doc|txt|png|jpg|jpeg|webp)$/i)) { label = "📄 문서"; bg = "#fff8e1"; color = "#f57c00"; }
                      else if (s.startsWith("http")) { label = "🌐 URL"; bg = "#f3e5f5"; color = "#7b1fa2"; }
                      else { label = "📎 기타"; bg = "#f5f5f5"; color = "#555"; }
                      return <span style={{ background: bg, color, padding: "2px 8px", borderRadius: 10, fontWeight: "bold" }}>{label}</span>;
                    })()}
                    {f.source && f.source !== "manual" && (
                      <span style={{ color: "#bbb" }} title={f.source}>
                        {f.source.length > 40 ? f.source.slice(0, 40) + "..." : f.source}
                      </span>
                    )}
                  </div>
                )}
