...
export default function Home() {
  const [messages, setMessages] = useState([
    { role: "bot", text: "안녕하세요! 닥터비트사업부 고객지원 AI입니다 😊\nU차트, A차트, 비트플러스 관련 문의를 도와드립니다.\n편하게 사용해 주세요!", time: getTime() },
  ]);
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: "bot", text: reply, time: getTime(), question: msg, feedback: null }]);
    } catch {
              ...
              <div style={{ maxWidth: "75%" }}>
                ...
                {m.role === "bot" && m.question && (
                  <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
                    {m.feedback === null ? <>
                      <button onClick={() => sendFeedback(i, true)} style={fbBtn("#e8f5e9", "#2e7d32")}>👍 도움됐어요</button>
                      <button onClick={() => sendFeedback(i, false)} style={fbBtn("#ffebee", "#c62828")}>👎 별로예요</button>
                    </> : m.feedback === "good"
                      ? <span style={{ fontSize: 11, color: "#2e7d32" }}>👍 피드백 감사해요!</span>
                      : <span style={{ fontSize: 11, color: "#c62828" }}>👎 더 나은 답변을 준비할게요!</span>
                    }
                  </div>
                )}
              </div>
...
const fbBtn = (bg, color) => ({ background: bg, color, border: "none", borderRadius: 10, padding: "3px 10px", fontSize: 11, cursor: "pointer", fontFamily: "inherit" });
